...
out_dir <- file.path(base_dir, "3_GSEA", "3_GSEA_KEGG_STAT")
...
save_outputs <- function(prefix, subtitle, res_list) {
  ...
  write_tsv(res_list$full,
            file.path(out_dir, paste0(prefix, "_KEGG_STAT_FULL.tsv")))
  write_tsv(res_list$sig,
            file.path(out_dir, paste0(prefix, "_KEGG_STAT_SIG_FDR0.05.tsv")))
...
save_outputs("Ovary_control_GTEx", "Ovary tumors vs ovary control GTEx", res_raw)

# ===================== RUN REFERENCE ===================== #
res_ae <- run_gsea_kegg(make_rank_stat(DE_ae))
save_outputs("Reference_control", "Ovary tumors vs reference control", res_ae)
