...
expr_raw_all_int <- readRDS("3_0_1_expr_raw_autoGTEX.rds")
...
meta_auto <- metadata_all %>%
  filter(group_pca %in% c("TCGA_tumor_ovary", "control_autoencoder_GTEx")) %>%
  mutate(
    condition = ifelse(group_pca == "TCGA_tumor_ovary",
...
process_group(
  ...
  dds_file      = "3_1_3_dds_Tumor_vs_GTEx_autoencoder.rds",
  vsd_file      = "3_1_4_vsd_Tumor_vs_GTEx_autoencoder.rds",
  pca_file      = "3_1_4_PCA_vst_Tumor_vs_GTEx_autoencoder.pdf"
)
